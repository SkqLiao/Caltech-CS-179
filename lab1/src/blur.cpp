...
int large_gauss_test(int argc, char **argv) {
  ...
  {
    float total = 0.0;
    for (int i = 0; i < GAUSSIAN_SIZE; i++) total += blur_v[i];
    for (int i = 0; i < GAUSSIAN_SIZE; i++) blur_v[i] /= total;
  ...
  amt_read = sf_read_float(in_file, all_channel_input,
  ...
  for (int ch = 0; ch < n_channels; ch++) {
    ...
    const unsigned int blocks =
        std::min(max_blocks, (unsigned int)ceil(n_frames / (float)local_size));
    ...
    if (success) cout << endl << "Successful output" << endl;
    ...
    cout << endl
         << "Speedup factor: " << cpu_time_milliseconds / gpu_time_milliseconds
         << endl
         << endl;
...
int main(int argc, char **argv) { return large_gauss_test(argc, argv); }
